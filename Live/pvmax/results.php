    ...
    <div class="tab-content">
    	...
    	<div class="col-sm-7">
        	<div class="row">
                ...
                // echo "<td>6.2 m S1.5 Rail</td>";
                // echo "<td class='qty'>".$CuttingStockRail ."</td>";
                // echo "</tr>";
                ...
                echo "</tr>";
                 ...
                 echo "<td>PvMax Support Triangle ".$tilt." degrees</td>";
...
<td>
<?php
if($IntermediateRailRow == 0 ){echo 0;}else{
...
<td>&commat;</td>
<td><?php
if($sysLength<= $stockLen){echo 0 ;}elseif($support == 2){echo 0;}else{echo $roundendcut= round($bestresults["endRailLength"],-1);}?>
      ...
      <div id="dialogmsg" title="login to add to basket" style="display:none">
    	...
    	<div class="col-sm-7">
        	<div class="row">
                ...
                echo "   <tr>";
                   echo "      <th>Type of Rack</th>";
                   echo "      <td>";
                   if($type_of_rack == 'rammed-post'){echo "Rammed";}else{echo "Foundation";};
                   echo "</td>";
                   echo "      <th>Ground Snow</th>";
                    echo "      <td>".$snow." psf</td>";
                echo "</tr>";

                echo "   <tr>";
                   echo "      <th>Size of Rack</th>";
                   echo "      <td>2V x ".$moduleCount."</td>";
                   echo "      <th>System Length</th>";
...
echo '<tr>';
echo '<th>Max Span</th>';
echo '<td>'.$key['max_span'].' ft</td>';
echo '<th>Ballast Width</th>';
echo '<td>'.$key['ballast_width'].' in</td>';
echo '<th>Rebar Qty</th>';
echo '<td>'.$key['rebar'].' </td>';
echo '</tr>';
echo '<tr>';
echo '<th>FCT</th>';
echo '<td>'.round($key['front_conne_tension'],2).' kip</td>';
echo '<th>FCC</th>';
echo '<td>'.round($key['front_conne_compression'],2).' kip</td>';
echo '<th>FCS</th>';
echo '<td>'.round($key['front_conne_shear'],2).' kip</td>';
echo '</tr>';
echo '<tr>';
echo '<th>RCT</th>';
echo '<td>'.round($key['rear_conne_tension'],2).' kip</td>';
echo '<th>RCC</th>';
echo '<td>'.round($key['rear_conne_compression'],2).' kip</td>';
echo '<th>RCS</th>';
echo '<td>'.round($key['rear_conne_shear'],2).' kip</td>';
echo '</tr>';

 $_SESSION['code710maxspan']    =  $key['max_span'];
 $_SESSION['code710blockwidth'] =  $key['ballast_width'];
 $_SESSION['code710rebar']      =  $key['rebar'];
 $_SESSION['code710front_conne_tension'] = $key['front_conne_tension'];
 $_SESSION['code710front_conne_comp']    = $key['front_conne_compression'];
 $_SESSION['code710front_conne_shear']   = $key['front_conne_shear'];
 $_SESSION['code710rear_conne_tension']  = $key['rear_conne_tension'];
 $_SESSION['code710rear_conne_comp']     = $key['rear_conne_compression'];
 $_SESSION['code710rear_conne_shear']    = $key['rear_conne_shear'];
...
echo '</tr>';
 ...
 $_SESSION['code705maxspan']    =  $key['max_span'];
 $_SESSION['code705blockwidth'] =  $key['ballast_width'];
 $_SESSION['code705rebar'] 	    = $key['rebar'];
 $_SESSION['code705front_conne_tension'] = $key['front_conne_tension'];
 $_SESSION['code705front_conne_comp']    = $key['front_conne_compression'];
 $_SESSION['code705front_conne_shear']   = $key['front_conne_shear'];
 $_SESSION['code705rear_conne_tension']  = $key['rear_conne_tension'];
 $_SESSION['code705rear_conne_comp']     = $key['rear_conne_compression'];
 $_SESSION['code705rear_conne_shear']    = $key['rear_conne_shear'];
...
}else if($module_cell == 72 && $codeVersion == 'ASCE7-10'){
...
RCS : Rear Connection Shear</p>
        ...
        <input type="text" name="projectaddress" placeholder="Project Address" value="<?php if(isset($_SESSION['projectAddress'])) echo $_SESSION['projectAddress']?>"/>
                    ...
                    <div class="col-sm-4">
                    <input type="number" name="zipcode" placeholder="Zip" value="<?php if(isset($_SESSION['projectZip'])) echo $_SESSION['projectZip']?>"/>
                    </div>
                    </div>
...
<?php
  $_SESSION['pvmaxRailCount']            =        $CuttingStockRail;
  ...
  $_SESSION['pvmaxTriangleItem']         =        $triangleItem;
  $_SESSION['pvmaxSuppTriCount']         =        $supportTriCount;
  $_SESSION['pvmaxEndClampitemnum']      =        $endClamp['item_number'];
  $_SESSION['pvmaxEndClampdesc']         =        $endClamp['description'];
  $_SESSION['pvmaxEndClampqty']          =        $endclampqty;
  $_SESSION['pvmaxMiddleClampitemnum']   =        $middleClamp['item_number'];
  $_SESSION['pvmaxMiddleClampdesc']      =        $middleClamp['description'];
  $_SESSION['pvmaxMiddleClampqty']       =        $middleClampqty;
  $_SESSION['pvmaxLaminateHookdesc']     =        $laminatehook['description'];
  $_SESSION['pvmaxLaminateHookItemNum']  =        $laminatehook['item_number'];
  $_SESSION['pvmaxLaminateHookqty']      =        $laminatehookqty;
  $_SESSION['pvmaxsystemlength']         =        $systemLengthIn;
  $_SESSION['pvmaxSpliceCount']          =        $spliceCountTotal;
  $_SESSION['stocklen']                  =        $stockLen;
  $_SESSION['sysLength']                 =        $sysLength;
  $_SESSION['sysLengthft']				 = 		  $sysLengthft;
  $_SESSION['support']                   =        $support;
  //session variable for pdf
  $_SESSION['pvmacenterCount']           =        $bestresults["centerRailCount"];
  $_SESSION['pvmacenter']                =        round($bestresults["centerRailLength"],-1);
  $_SESSION['pvmaIntermRailRow']         =        $bestresults["intermediateRailCount"];
  $_SESSION['pvmaIntermRailCut']         =        round($bestresults["intermediateRailLength"],-1);
  $_SESSION['pvmaEndRailRow']            =        $bestresults["endRailCount"];
  $_SESSION['pvmaEndRailCut']            =        round($bestresults["endRailLength"],-1);
  $_SESSION['pvmaRailColumnTF']          =        $RailColumnTF;
  $_SESSION['pvmatotalRailLen']          =        $totalRailLen;
  $_SESSION['pvmatotalRailRow']          =        $totalRailRow;
  $_SESSION['pvmaIntermRowtotal']        =        $IntermRowTotal;
  $_SESSION['pvmaroundendcut']           =        $roundendcut;
  $_SESSION['pvmaroundIntercut']         =        $roundIntercut;
  $_SESSION['pvmaroundcenter']           =        $roundcenter;
  $_SESSION['pvmacentertotal']           =        $centerTotal;
  $_SESSION['pvmaendrowtotal']           =        $EndRowTotal;
  $_SESSION['pvmaspan']                  =        $span;
  $_SESSION['pvmaxonespanrail1'] 	     = 	      $onespanrail[0];
  $_SESSION['pvmaxonespanrail2'] 	     =        $onespanrail[1];
  $_SESSION['pvmaxcutdesc'] 			 = 		  $bestresults["instructions"];
  $_SESSION['intermcheck']               =		  $IntermediateCheck;
  $_SESSION['lowestRailLenCut'] 		 = 		  $lowestRailLenCut[0];
  //var_dump($_SESSION);

  $partsList = array(0,1,2,3,4,5);
$partsList[0] = array("partNumber" => "124303-06200","Description" =>"S1.5 Rail 6200","Quantity" => $bestresults["stockRailQuantity"][6200] );
$partsList[1] = array("partNumber" => "124303-04200","Description" =>"S1.5 Rail 4200","Quantity" => $bestresults["stockRailQuantity"][4200] );
$partsList[2] = array("partNumber" => $triangleItem,"Description" =>"PvMax Support Triangle $tilt degrees","Quantity" => $supportTriCount);
$partsList[3] = array("partNumber" => "129303-000","Description" =>"S1 Splice Kit","Quantity" => $spliceCountTotal);
$partsList[4] = array("partNumber" => $endClamp['item_number'],"Description" =>$endClamp['description'],"Quantity" => $endclampqty);
$partsList[5] = array("partNumber" => $middleClamp['item_number'],"Description" =>$middleClamp['description'],"Quantity" => $middleClampqty);
  $_SESSION["partslist"] = $partsList;

 if(isset($code_7_10_60)){
 	$_SESSION['code'] = $code_7_10_60;
  }elseif( isset($code_7_10_72)){
  	$_SESSION['code'] = $code_7_10_72;
  }elseif(isset($code_7_05_60)){
  	$_SESSION['code'] = $code_7_05_60;
  }elseif(isset($code_7_05_72)){
  	$_SESSION['code'] = $code_7_05_72;
  }

  //echo print_r($_SESSION['code']);
?>
               ...
               beforeSend: function (request)
               {
                // request.setRequestHeader('Accept','text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8');
			  	...
			  	success: function(msg) {
   			 	...
   			 	error: function(msg) {
        	      console.log("pdf test error" + msg);
    			},
...
$(document).ready(function() {
     ...
     $('#addPartsBtn').removeClass( 'disabled' );
       var store_url = window.location.protocol + '//' + document.domain + '/mm5/merchant.mvc?Screen=BASK&System=PvMax&Name=' + '<?php echo $_SESSION['projectname'];?>'
      var done = false;
      $('#basket-add').submit(function(event) {
      	//alert("Please clear the basket before continuing.");
