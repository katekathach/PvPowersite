 ...
 $(document).on('click', '.function_edit ', function(e){
      ...
      if (output.message == 'success'){
      	$("#saveProjectBtn").hide();
  			...
  			$("input[name='moduleColumns']").val(output.data.project_column)
  			
  			//if(output.data.project_building_code == "10") {
  			//	codeversion = "ASCE7-10";
  			//}else{
  			//	codeversion = "ASCE7-05";
  			//}
  			
  			$("#codeVersion option:contains('" + output.data.project_building_code + "')").prop('selected', true)
  			$( "#wind option:contains('"+output.data.project_wind_load +"')").prop('selected', true)
  			$( "#snow option:contains('"+output.data.project_snow_load +"')").prop('selected', true)
  			$("#seismic").val(output.data.project_seis)
  			
  			//$("#wind").html("<option value='" + output.data.project_wind_load + "'>"+output.data.project_wind_load+"</option>")
  			//$("#snow").html("<option value='" + output.data.project_snow_load + "'>"+output.data.project_snow_load+"</option>")
  			
  			$("input[name='numberRacks']").val(output.data.project_num_racks)
             } else {
            hide_loading_message();
